...
public Adminlog AdminLogin(String username, String password) throws AdminException  {
	...
	try(Connection conn = DBUtil.provideConnection()) {
		...
		PreparedStatement ps= conn.prepareStatement("select * from adminlog where email = ? AND password = ?");			
...
public List<StudentDTO> getCourseDetailsAndBatch() throws CourseException {
	...
	try (Connection conn=DBUtil.provideConnection()){
		PreparedStatement ps=conn.prepareStatement("select s.roll,s.name,s.email,a.cname,a.fees,a.cid,b.bid,b.totalseat from student s INNER JOIN administrator a INNER JOIN batch b INNER JOIN studentDTO sd ON s.roll=sd.csroll AND\r\n"
				+ "a.cid=b.bid ;");
...
public List<Administrator> GetDetailsCourse() throws CourseException {
	...
	try (Connection conn=DBUtil.provideConnection()){
		...
		while (rs.next()) {
			...
			Administrator administrator=new Administrator(o,n,j);
